...
def get_parser():
    ...
    # visulize and debug
    parser.add_argument('--save_epoch', type=int, default=138, help='the start epoch to save model (#iteration)')
    ...
    # pretrained models
    parser.add_argument('--pretrained_vision_model', default=None, help='the model will be used')
    parser.add_argument('--pretrained_vision_model_config', default='./config', help='the arguments of model')
    parser.add_argument('--enable_dotmap', type=str2bool, default=False, help='')
    parser.add_argument('--model_type', default='video', help='')
    parser.add_argument('--align_channel', type=int, default=[768, 256])
    parser.add_argument('--balance', type=float, default=[0.7, 0.4])
    parser.add_argument('--refinement', type=float, default=0.8)
...
class Processor():
    ...
    def __init__(self, arg):
        ...
        if self.arg.pretrained_vision_model:
            self.vision_model = self.vision_model.cuda(self.output_device)

        if type(self.arg.device) is list:
            if len(self.arg.device) > 1:
                self.model = nn.DataParallel(self.model, device_ids=self.arg.device, output_device=self.output_device)
                if self.arg.pretrained_vision_model:
                    self.vision_model = nn.DataParallel(self.vision_model, device_ids=self.arg.device, output_device=self.output_device)
    ...
    def load_model(self):
        ...
        if self.arg.pretrained_vision_model:
            ...
            self.model = Model(align_mlp=Align(self.arg.align_channel[0], self.arg.align_channel[1]), **self.arg.model_args)
        else:
        ...
        if self.arg.pretrained_vision_model:
            with open(self.arg.pretrained_vision_model_config, 'r') as f:
                cfg = yaml.load(f, yaml.FullLoader)

            Pretrained_Vision_Model = import_class(self.arg.pretrained_vision_model)

            if not self.arg.enable_dotmap:
                self.vision_model = Pretrained_Vision_Model(
                    **cfg
                )
            else:
                self.vision_model = Pretrained_Vision_Model(
            ...
            self.print_log('Model load finished: ' + self.arg.pretrained_vision_model)
        else:
            self.print_log('No pretrained vision model')
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch, (data, video, label, sample) in enumerate(tqdm(self.data_loader['train'], desc="Training", ncols=100)):
            self.global_step += 1
            with torch.no_grad():
                data = data.float().cuda(self.output_device)
                label = label.long().cuda(self.output_device)
                if self.arg.pretrained_vision_model:
                    video = video.to(torch.float32).cuda(self.output_device)
  
            # forward
            with torch.no_grad():
                if self.arg.pretrained_vision_model:
                    if self.arg.model_type == 'video':
                        vision_feature = self.vision_model(video)
                    elif self.arg.model_type == 'image':
                        N, C, T, H, W = video.shape
                        vision_feature = self.vision_model(video.permute(0, 2, 1, 3, 4).contiguous().view(N * T, C, H, W)) # output: N*T, C, H, W
                        vision_feature = vision_feature.view(N, T, -1).mean(1)
                    elif self.arg.model_type == 'sapiens':
                        N, C, T, H, W = video.shape
                        (vision_feature,) = self.vision_model(video.permute(0, 2, 1, 3, 4).contiguous().view(N * T, C, H, W)) # output: N*T, C, H, W
                        vision_feature = vision_feature.mean(-1).mean(-1).view(N, T, -1).mean(1)
            ...
            if p < 0.5:
                # normal
                if self.arg.pretrained_vision_model:
                    output, refine, align_loss = self.model(data, vision_feature)
                    vision_loss = self.arg.balance[1] * align_loss.mean()
                    loss = self.loss(output, label) + self.arg.balance[0] * self.loss(refine, label) + vision_loss
                else:
                ...
                if self.arg.pretrained_vision_model:
                    mixed_vision_feature = lam * vision_feature + (1 - lam) * vision_feature[index, :]
                    output, refine, align_loss = self.model(mixed_data, mixed_vision_feature)
                    vision_loss = self.arg.balance[1] * align_loss.mean()
                    loss = lam * self.loss(output, label) + (1 - lam) * self.loss(output, label[index]) \
                        + self.arg.balance[0] * (lam * self.loss(refine, label) + (1 - lam) * self.loss(refine, label[index])) \
                        + vision_loss
                else:
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            label_list = []
            pred_list = []
            for batch, (data, _, label, sample) in enumerate(tqdm(self.data_loader[ln], desc="Evaluating", ncols=100)):
                ...
                with torch.no_grad():
                    ...
                    if self.arg.pretrained_vision_model:
                        output_origin, refine_output = self.model(data)

                        refinement = self.arg.refinement
                        output = refinement * output_origin + (1.0 - refinement) * refine_output
                    else:
                        output = self.model(data)
