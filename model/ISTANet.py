...
# STAR
class ModelAlign(nn.Module):
    def __init__(self, align_mlp, window_size, num_classes, num_joints, 
                 num_frames, num_persons, num_heads, num_channels, 
                 kernel_size, use_pes=True, config=None, 
                 att_drop=0, dropout=0, align_layer=5, align_num_channels=768):
        super().__init__()
        ...
        self.align_layer_channel = config[align_layer][1]
        self.align_fc = nn.Linear(align_num_channels, num_classes)

        for m in self.modules():
            if isinstance(m, nn.Linear):
                fc_init(m)

    def forward(self, x, vision_feature=None):
        ...
        for i, block in enumerate(self.blocks):
            x = block(x)
            if self.align_mlp and i == self.align_layer:
                align_feature = x.view(N, self.align_layer_channel, -1).permute(0, 2, 1).contiguous()
                align_feature = align_feature.mean(1)
                if self.training:
                    align_loss, vision_feature = self.align_mlp(align_feature, vision_feature)
                    refine = self.align_fc(vision_feature)

        x = x.view(N, self.out_channels, -1).permute(0, 2, 1).contiguous()
        x = x.mean(1)
        x = self.drop_out(x)

        if self.training and self.align_mlp:
            return self.fc(x), refine, align_loss
        else:
            return self.fc(x), self.align_fc(align_feature)
